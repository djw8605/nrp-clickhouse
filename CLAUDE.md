...
1. Four instant PromQL queries anchored with `@end_ts` subqueries over `[1d:5m]`: pod resource requests (joined against `kube_pod_status_phase{phase="Running"}` — see gotchas), pod annotations (user attribution), DCGM GPU utilization (GPU model names), and pod wall hours (`kube_pod_status_phase{phase="Running"}` summed rather than used as a filter, stored as `resource='wall'`). Plus one LLM token query (`gen_ai_client_token_usage_sum`).
2. Enrichment joins happen client-side in Python, not in PromQL: `attach_pod_annotations_to_resource_payload` and `attach_gpu_model_names_to_resource_payload` merge labels into the resource payload. `attach_node_labels_to_payload` copies `node` onto the wall payload, which needs it because `kube_pod_status_phase` has no `node` label — see gotchas.
...
- Prometheus results with a missing `node` label fall back to the `instance` label in `_extract_node_label`, which for kube-state-metrics is the KSM pod IP — "nodes" like `10.244.x.x` in old data come from this. `kube_pod_status_phase` (the wall-hours source) is exactly such a metric, so `attach_node_labels_to_payload` sets `node` from the resource payload and pins unmatched pods to `unknown` rather than letting the fallback fire.
- **`resource='wall'` is not billable.** It is pod wall hours, the denominator that turns core-hours back into device counts (`CPU = cpu_hours / wall_hours`) for the XDMoD upload. Any new query that sums `usage` must filter or group by `resource` — `accounting_queries._validate_resource_safety` enforces this, so don't work around it.
- **The wall-hours query must keep its `max by(namespace, pod, uid)`.** `kube_pod_status_phase` carries a `container` label naming the KSM scrape container, so a pod can appear on several series and a bare `sum_over_time` would double-count its wall time.
- **XDMoD uploads for dates ingested before 2026-07-28 have no wall row.** `xdmod_upload` falls back to a 24-hour day and logs `xdmod_upload_wall_hours_missing`; those dates report device counts averaged over the day until re-run with `--force`.
